...
class MainVerticle : AbstractVerticle() {
  ...
  override fun start(startPromise: Promise<Void>) {
    val resource = Resource.getDefault().toBuilder()
      .put(AttributeKey.stringKey("service.name"), "service-a")
      .build()

    val otel = configure(resource)
    tracer = otel.getTracer("main-verticle")
    ...
    router.get("/").handler { ctx ->
      val span = tracer.spanBuilder("external-api-call").startSpan()

      client.get(8080, "service-b", "/")
        .send()
        .onSuccess { response ->
          span.end()
